...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const token = res.data.token || res.data.idToken || null;
      const user = res.data.user || res.data.profile || { uid: res.data.uid, email: res.data.email };

      if (token) localStorage.setItem("token", token);
      localStorage.setItem("user", JSON.stringify(user));

      addNotification(`Welcome back${user?.profile?.username ? ", " + user.profile.username : ""}!`, "success", 3000);

      setTimeout(() => navigate("/dashboard"), 1500);
    } catch (err) {
      console.error("Login failed:", err.response?.data || err.message);
      const errorMsg = err.response?.data?.message || err.response?.data?.msg || err.message || "Login failed!";
      addNotification(errorMsg, "error", 5000);
      setErrors({ submit: errorMsg });
    } finally {
      setIsLoading(false);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white p-4">
      <div className="bg-black/40 p-8 rounded-2xl shadow-lg w-full max-w-md border border-gray-700/50">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
              />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-gray-400 hover:text-gray-200 text-lg">
                {showPassword ? "👁️" : "👁️‍🗨️"}
              </button>
            </div>
          ...
          <button type="submit" disabled={isLoading} className={`w-full py-3 rounded-lg font-bold transition mt-2 ${isLoading ? "bg-gray-600 cursor-not-allowed opacity-50" : "bg-blue-600 hover:bg-blue-700"}`}>
            {isLoading ? "Logging in..." : "Login"}
          </button>
        </form>
        ...
        <div className="mt-6 space-y-3 text-center text-gray-400">
          ...
          <p className="text-xs text-gray-500">
            Use demo credentials to test: <span className="text-gray-400">email: test@test.com</span>
          </p>
        </div>
      </div>
    </div>
